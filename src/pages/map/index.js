...
class MapPage extends React.Component {
  ...
  getPlace = async (code) => {
    ...
    if (data) {
      ...
      this.getMapData(geoJson, code)
    }
    this.setState({ loading: false })
  }

  getMapData = (geoJson, code) => {
    ...
    const pointData = code === 'B000A7BD6C' ? this.handleTsinghuaPoint() : this.handlePointData()
    this.lastDirect = {
      adcode: parentInfo[parentInfo.length - 1].cityCode,
      geoJson,
    }
    this.setState({ geoJson, mapData, pointData, loading: false })
  }

  handlePointData = () => {
    const { parentInfo } = this.state
    const result = []
    if (parentInfo[parentInfo.length - 1].cityName === '清华大学') {
      return result
    }
    placeData.forEach((e) => {
      if (!_.find(result, { 所在省份: e['所在省份'] }) && parentInfo.length === 1) {
        result.push(e)
      } else if (parentInfo.length === 2 && e['所在省份'] === parentInfo[1].cityName && result.length <= 5) {
        result.push(e)
